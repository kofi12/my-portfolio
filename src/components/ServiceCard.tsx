...
export function ServiceCard({ icon, heading, description }: ServiceCardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative rounded-2xl border border-white/40 bg-white/40 shadow-[0_4px_24px_rgba(0,0,0,0.10),0_1.5px_4px_rgba(59,130,246,0.10)] p-6 flex flex-col items-center w-1/4 max-w-1/6 md:w-1/4 sm:w-full gap-4 transition-all duration-300 overflow-hidden"
    ...
    >
      ...
      <h2 className="font-heading text-2xl font-bold text-slate-700 mb-2 text-center relative z-10">
        {heading}
      </h2>
      <p className="font-para text-base text-slate-600 text-center relative z-10">
        {description}
      </p>
    </motion.div>
  );
